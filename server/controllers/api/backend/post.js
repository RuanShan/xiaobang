...
module.exports=class Posts {
  ...
  static async index(ctx) {
    ...
    let options = Object.assign( { include:[{association: 'Terms'}]}, pagination )
  ...
  static async addPost(ctx) {
    ...
    let post = await PostModel.create(postAttributes,{
      fields: ['author', 'title', 'created_by', 'type', 'start_at', 'end_at', 'publish_at','content']
    })
  ...
  static async modifyPost(ctx) {
    try {
      ...
      post = await post.update(postAttributes,{
        fields:['title', 'desc', 'author','content', 'publish_at']
      })

...
const CounterRunner = require('../../models/dpgame/counter/runner')
const {
  DpGameRoundStates
...
function createInterval(gameRound, time, namespace,runner) {
  let runningTimeId = setInterval(function() {
    let gameState = redis.get(gameRound.number)
    ...
    runner.getPlayerScores().then((players) => {
      ...
      logger.info(`......socket:broadcast->${namespace.name}:startGame${time} - ${cachedPlayers.length}`)
    }).catch(function(err) {
...
module.exports = class DpCounterSocket {
  // bind on connection
  static async bind(io) {
    const dynamicNsp = io.of(/^\/channel-dpcounter-\w+$/).on('connect', (socket) => {
      ...
      DpCounterSocket.bindPlay(socket, number);
      DpCounterSocket.bindPlayWx(socket, number);
  ...
  static async bindPlayWx(socket, number) {
    ...
    //   let game_round_id = CounterSocket.getGameRoundId( ctx )
    //   let runner =  new CounterRunner(game_round_id)
    //
    ...
    socket.on('ShakeEvent', async (data, callback) => {
      ...
      let runner = new CounterRunner(number)
      let game_player_id = data.game_player_id
      let score = data.score
      ...
      if (gameRound.score != 0 && score > gameRound.score) {
        runner.endRound().then(() => {
          runner.getAllPlayers().then((players) => {
            players.sort((a, b) => {
              return b.score - a.score
    ...
    socket.on('StartGameEvent', async (data, callback) => {
      ...
      var countTimeId = setInterval(function() {
        ...
        if (countTime < 1) {
          ...
          // 触发客户端的开始游戏事件
          redis.set(gameRound.number, "started");
          if (data == 'duration') {
            var runningTimeId = createInterval(gameRound, runningTime, namespace,runner)
          } else if (data == 'score') {
            var runningTimeId = createInterval(gameRound, 7200, namespace,runner)
          } else if (data == 'both') {
            var runningTimeId = createInterval(gameRound, runningTime, namespace,runner)
          }
          callback({

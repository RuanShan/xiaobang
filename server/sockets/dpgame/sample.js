...
const Runner  =require(  '../../models/dpgame/sample/runner')
const {
  DpGameRoundStates
}  =require(  '../../models/constant')

module.exports=class DpSampleSocket {
  // bind on connection
  static async bind(io) {
    const dynamicNsp = io.of(/^\/channel-dpsample-\w+$/).on('connect', (socket) => {
      ...
      DpSampleSocket.bindPlay(socket, number);

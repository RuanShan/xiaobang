...
class CounterRunner {
  ...
  async GetRoundAllPlayers() {
    ...
    let gamePlayers = await DpCounterGamePlayer.findAll({
      where: {
  ...
  async AddPlayersToMemoryDb() {
    ...
    let round = await this.getGameRound()
    let gameroundid = round.id
    let game_round = await DpCounterGameRound.findByPk(gameroundid)
  ...
  async updatePlayerScore(playerid, score) {
    ...
    redis.hgetall(key, (error, players) => {
      console.log('updatePlayerScore-----------------:',players.length);
    })
  ...
  async insertGameResult(playerid, score) {
    ...
    let gameResult = DpCounterGameResult.build(gameResultParams)
  ...
  async getRedisPlayerScores(gameroundid) {
    ...
    var p = new Promise((resolve) => {
      redis.hgetall(key, (error, players) => {
        console.log('players-----------------:', gameroundid, players.length);
  ...
  async getAllPlayers() {
    ...
    console.log('getAllPlayers----:',gamePlayers.length);
    return gamePlayers
  }
  /**
   * 取得玩家信息，在数据库取得，以便显示玩家列表
  ...
  async getAllResults() {
    ...
    let round = await this.getGameRound()
    let gameroundid = round.id
    let gameResults = await DpCounterGameResult.findAll({
      where: {
        game_round_id: gameroundid
      },
      include: [{
  ...
  async endRound() {
    ...
    for (var i = 0; i < players.length; i++) {
      ...
      let res = DpCounterGamePlayer.update({
        rank: i,
        'score': player.score
      }, {
        where: {
    ...
    await DpCounterGameRound.update({
      state: state,
  ...
  async resetRound() {
    ...
    let res = await DpCounterGameResult.update({
      deleted_at: now
    }, {
      where: {
  ...
  async getGameRound() {
    return await DpCounterGameRound.findOne({
      where: {
        number: this.number
      }
    })

...
class QiandaoRunner {
  ...
  async GetRoundAllPlayers() {
    ...
    let gamePlayers = await DpQiandaoGamePlayer.findAll({
      where: {
  ...
  async AddPlayersToMemoryDb() {
    let round = await this.getGameRound()
    let gameroundid = round.id
    let game_round = await DpQiandaoGameRound.findByPk(gameroundid)
  ...
  async endRound() {
    ...
    await DpQiandaoGameRound.update({
      state: state,
  ...
  async getGameRound() {
    return await DpQiandaoGameRound.findOne({
      where: {
        number: this.number
      }
    })

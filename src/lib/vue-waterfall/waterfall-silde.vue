...
export default {
  ...
  methods: {
    ...
    async getHeight(i) {
      await this.imgLoad()
      // 当查询时，刷新结果，可能图片加载过了，但是内容没有生成，这时this.$refs.waterfallSilde 为null
      console.debug( "this.$refs.waterfallSilde=", this.objid)
  ...
  mounted() {
    console.debug( "waterfall-silde mounted", this.objid)

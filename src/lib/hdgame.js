...
import { initWxConfig, setWxShare } from './hdgame/weixin'
...
HdGame.initJsHead = function(hg, _data) {
  hg.assets = (function() {
    ...
    let assets = {
      ...
      loadPage: function() {
        ...
        console.debug(" loadStart 0 groups", groups);
          ...
          loadimg(group.path,
            function() {
              ...
              if (++groupLoaded < groups.length) {
                ...
                _this.complete = true;
                console.debug("eventBus.fire(ready)")
                eventBus.fire("ready")
              }
              if (group.name === FIRST_NAME) {
                console.debug("home is loaded!");
                checkOtherLoaded()
              }
            })
        })(0);
        ...
        function loadimg(arr, callBack) {
          ...
          function loadCheckComplete() {
            ...
            // } else {
            //   console.debug(this, "assets_key is undefined!")
            // }
  ...
  (function() {
    ...
    for (var i = 0; i < originDef.length; i++) {
      console.debug("edit","i="+i, origin[i], originDef[i])
  ...
  // 加载资源
  (function() {
    let assetsImage = [_data.logoImgPath, _resRoot + "/image/ruleImg.png", _resRoot + "/image/success.png", _resRoot + "/image/light.png", _resRoot + "/image/musicOff.png", _resRoot + "/image/musicOn.png", g_config.headImg];
    ...
    if( _data.skinAssets){
      ...
      if( _data.skinAssets.homeBgPath && _data.skinAssets.titleImgPath){
        hg.assets.add("home", [ _data.skinAssets.homeBgPath, _data.skinAssets.titleImgPath]);
      }
    }
...
HdGame.initEdit = function(Edit) {
  console.debug(" doing init edit =", Edit);
...
HdGame.initCallBack = function(target, arg) {
  ...
  target = target || {};
  console.debug("callBackObj=", callBackObj)
...
HdGame.getPosAndSize = function(theObj, def, type) {
  ...
  //console.debug(`type=${type}, theObj=${theObj.width}:${theObj.width}, def=${def.width}:${def.height},`)
...
import{ isHttps } from './hdgame/url'
HdGame.isHttps = isHttps
